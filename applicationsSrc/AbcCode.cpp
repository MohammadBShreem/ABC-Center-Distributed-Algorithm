...
AbcCode::AbcCode(BlinkyBlocksBlock *host):BlinkyBlocksBlockCode(host),module(host) {
    ...
    addMessageEventFunc2(GO_MSG_ID_A,std::bind(&AbcCode::myGoFuncFromA,this,std::placeholders::_1, std::placeholders::_2));
    addMessageEventFunc2(BACK_MSG_ID_A,std::bind(&AbcCode::myBackFuncFromA,this,std::placeholders::_1, std::placeholders::_2));
    addMessageEventFunc2(LIGHT_MSG_ID_A,std::bind(&AbcCode::myLightFuncFromA,this,std::placeholders::_1, std::placeholders::_2));

    addMessageEventFunc2(GO_MSG_ID_B,std::bind(&AbcCode::myGoFuncFromB,this,std::placeholders::_1, std::placeholders::_2));
    addMessageEventFunc2(BACK_MSG_ID_B,std::bind(&AbcCode::myBackFuncFromB,this,std::placeholders::_1, std::placeholders::_2));
    addMessageEventFunc2(LIGHT_MSG_ID_B,std::bind(&AbcCode::myLightFuncFromB,this,std::placeholders::_1, std::placeholders::_2));

    addMessageEventFunc2(LIGHT_MSG_ID_C,std::bind(&AbcCode::myLightFuncFromC,this,std::placeholders::_1, std::placeholders::_2));
...
void AbcCode::startup() {
    ...
    {
        setColor(Color(1.8f,1.8f,1.8f));
    }
}

void AbcCode::myGoFuncFromA(std::shared_ptr<Message> _msg, P2PNetworkInterface *sender) {
...
void AbcCode::myBackFuncFromA(std::shared_ptr<Message> _msg, P2PNetworkInterface *sender) {
    MessageOf<int>* msg = static_cast<MessageOf<int>*>(_msg.get());
    int msgData = *msg->getData();
...
void AbcCode::myLightFuncFromA(std::shared_ptr<Message> _msg, P2PNetworkInterface *sender) {
    if (myLongestChild) {
        sendMessage("Light", new Message(LIGHT_MSG_ID_A), myLongestChild, 100, 0);
    } else {
        setColor(YELLOW);
...
void AbcCode::myGoFuncFromB(std::shared_ptr<Message> _msg, P2PNetworkInterface *sender) {
    MessageOf<int>* msg = static_cast<MessageOf<int>*>(_msg.get());
    int msgData = *msg->getData();
...
void AbcCode::myBackFuncFromB(std::shared_ptr<Message> _msg, P2PNetworkInterface *sender) {
    MessageOf<int>* msg = static_cast<MessageOf<int>*>(_msg.get());
    int msgData = *msg->getData();
    console << "rcv ACK " << msgData << " from " << sender->getConnectedBlockBId() << "\n";
...
void AbcCode::myLightFuncFromB(std::shared_ptr<Message> _msg, P2PNetworkInterface *sender) {
    if (myLongestChildB) {
        sendMessage("Light", new Message(LIGHT_MSG_ID_B), myLongestChildB, 100, 0);
    } else {
        setColor(BLUE);
        ...
        //myNbWaitedAnswerC=sendMessageToAllNeighbors("go",new MessageOf<int>(GO_MSG_ID_C,1), 100, 1000, 0);
    }
}

void AbcCode::myLightFuncFromC(std::shared_ptr<Message> _msg, P2PNetworkInterface *sender) {
...
void AbcCode::parseUserBlockElements(TiXmlElement *config) {
    //Time t1 = scheduler->now() + 2 + (Time)(((double)1*hostBlock->getRandomUint())/((double)uintRNG::max()));
